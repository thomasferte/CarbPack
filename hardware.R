...
packages <- c("tidyverse" ,"benchmarkme", "sessioninfo", "sys",
              "readxl")
...
## il faut trouver le moyen de récupérer le TDP sachant que les processeurs
## sont variables en fonction des PC mais semblent être tous de la marque
## intel ou amd(piste ?)

## je n'ai pas les droits d'admin pour installer l'API d'intel
## on va estimer la valeur du TDP à 15W par coeur 
...
# un fichier pour pc fixe et un pour pc portable
cpu_intel_fixe<-readxl::read_excel(
  path = "./Intel-Core-Desktop-Boxed-Processors-Comparison-Chart.xlsx",
  sheet = 1,
  skip = 6)
cpu_intel_portable<-readxl::read_excel(
  path = "./Intel-Core-Comparsion.xlsx",
  sheet = 1,
  skip = 5)
cpu_amd<-readxl::read_excel(
  path = "./AMD processors.xlsx",
  sheet = 1)
...
if(length(cpu_version)>0) {
  if(cpu_version %in% cpu_intel_fixe$`Processor Number`) {
    cpu_TDP<- 
      cpu_intel_fixe$TDP[cpu_intel_fixe$`Processor Number` == cpu_version]
    cat("La version du processeur est présente. Valeur du TDP :", cpu_TDP, "W\n")
  } else { if(cpu_version %in% cpu_intel_portable$`Processor Number`) {
    cpu_TDP<- 
      cpu_intel_portable$TDP[cpu_intel_portable$`Processor Number` == cpu_version]
    cat("La version du processeur est présente. Valeur du TDP :", cpu_TDP, "W\n")
  } else { if(cpu_version %in% cpu_amd$`Processor Number`) {
    cpu_TDP<- 
      cpu_amd$TDP[cpu_amd$`Processor Number` == cpu_version]
    cat("La version du processeur est présente. Valeur du TDP :", cpu_TDP, "W\n")
  } else {
...
# début du décompte si tracker == TRUE
tracker<-TRUE
if(tracker == TRUE) {
 start_time <- Sys.time() 
}
